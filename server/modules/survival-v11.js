...
const WORLD = Object.freeze({ radius: 7, minY: -5, bedrockY: -6 });
...
function terrainTypeAt(gx, gy, gz) {
    ...
    if (gy <= -3 && r > 0.987) return 'diamond';
    if (gy <= -2 && r > 0.973) return 'emerald';
    if (gy <= -2 && r > 0.95) return 'amethyst';
    if (gy <= -1 && r > 0.91) return 'gold';
    if (r > 0.84) return 'iron';
...
function validateMineRequest({ blockType, blockKey, toolId, inventory = [] }) {
    ...
    if (position.gy === WORLD.bedrockY) return { ok: false, message: 'Không thể phá lớp đá nền cuối cùng.' };
...
function safeState(source = {}) {
    return {
        ...
        removedBlocks: Array.isArray(source.removedBlocks) ? source.removedBlocks.slice(-6000) : [],
