...
const TYPE_INFO={
 ...
 deepstone:{id:'survival_deepslate',name:'Đá sâu',color:'#45505c',hardness:3.4,tier:1,icon:'⬛'},
 foundation:{id:'survival_deepslate',name:'Móng đá cổ',color:'#2f3542',hardness:4,tier:2,icon:'🧱'},
 ...
 plank:{id:'survival_plank',name:'Ván gỗ',color:'#b8864f',hardness:1.3,tier:0,icon:'🟫'},
...
const RECIPES={
 planks:{name:'4 Ván gỗ',icon:'🟫',needs:'1 Gỗ thô',ingredients:{survival_log:1}},
 sticks:{name:'4 Gậy',icon:'🥢',needs:'2 Ván gỗ',ingredients:{survival_plank:2}},
 wood_pickaxe:{name:'Cuốc gỗ',icon:'⛏️',needs:'3 Ván + 2 Gậy',ingredients:{survival_plank:3,survival_stick:2}},
 stone_pickaxe:{name:'Cuốc đá',icon:'⛏️',needs:'3 Đá + 2 Gậy',ingredients:{survival_stone:3,survival_stick:2}},
 iron_pickaxe:{name:'Cuốc sắt',icon:'⚒️',needs:'3 Sắt + 2 Gậy',ingredients:{voxel_iron:3,survival_stick:2}},
 torch:{name:'4 Đuốc',icon:'🔥',needs:'1 Than + 1 Gậy',ingredients:{voxel_coal:1,survival_stick:1}},
 bread:{name:'Bánh quả rừng',icon:'🍞',needs:'3 Quả rừng',ingredients:{survival_berry:3}}
};
const ITEM_NAMES={survival_log:'Gỗ',survival_plank:'Ván',survival_stick:'Gậy',survival_stone:'Đá',voxel_iron:'Sắt',voxel_coal:'Than',survival_berry:'Quả rừng'};
let active=false;
let generated=false;
let terrain=[];
...
function ensureUI(){
 ...
 hud=document.createElement('aside');hud.id='survival-hud-v11';hud.innerHTML=`
  <div class="survival-title"><span>🏕️ Sinh tồn</span><b id="survival-level-v11">Cấp 1</b><button type="button" data-action="collapse" aria-label="Thu gọn bảng sinh tồn">−</button></div>
  <div class="survival-connection"><span id="survival-online-v12">● Đang kết nối</span><button type="button" data-action="reconnect">Kết nối lại</button></div>
 ...
 const craft=document.createElement('section');craft.id='survival-craft-v11';craft.setAttribute('aria-hidden','true');craft.innerHTML=`<header><b>🛠️ Bàn chế tạo kiểu Minecraft</b><button type="button" aria-label="Đóng">×</button></header><p>Chọn món cần làm. Chế độ nhanh sẽ tự làm Ván và Gậy từ Gỗ nếu ba lô đủ nguyên liệu.</p><label class="craft-auto-v14"><input id="craft-auto-v14" type="checkbox" checked><span>⚡ Tự chế tạo nguyên liệu trung gian</span></label><div id="craft-inventory-v14" class="craft-inventory-v14"></div><div class="craft-grid">${Object.entries(RECIPES).map(([id,r])=>`<button type="button" data-recipe="${id}"><span>${r.icon}</span><b>${r.name}</b><small>${r.needs}</small><em data-craft-state>Kiểm tra nguyên liệu</em></button>`).join('')}</div><button type="button" class="reset-world-v11">Tạo lại đảo sinh tồn</button>`;
 ...
 hud.querySelector('[data-action=reconnect]').addEventListener('click',reconnectServer);
 hud.querySelector('[data-action=collapse]').addEventListener('click',()=>{const collapsed=hud.classList.toggle('collapsed');hud.querySelector('[data-action=collapse]').textContent=collapsed?'+':'−';localStorage.setItem('survival-hud-collapsed-v12',collapsed?'1':'0')});
 if(localStorage.getItem('survival-hud-collapsed-v12')==='1'){hud.classList.add('collapsed');hud.querySelector('[data-action=collapse]').textContent='+'}
 ...
 craft.querySelector('#craft-auto-v14')?.addEventListener('change',renderCrafting);
...
function inventoryCountsLocal(){return (houseState?.inventory||[]).reduce((out,id)=>{out[id]=(out[id]||0)+1;return out},{})}
function canDirectCraft(recipe,counts){return Object.entries(recipe.ingredients||{}).every(([id,need])=>(counts[id]||0)>=need)}
function renderCrafting(){const panel=document.getElementById('survival-craft-v11');if(!panel)return;const counts=inventoryCountsLocal();const inv=document.getElementById('craft-inventory-v14');if(inv){const important=['survival_log','survival_plank','survival_stick','survival_stone','voxel_coal','voxel_iron','survival_berry'];inv.innerHTML=important.map(id=>`<span><b>${ITEM_NAMES[id]}</b> ${counts[id]||0}</span>`).join('')}panel.querySelectorAll('[data-recipe]').forEach(button=>{const recipe=RECIPES[button.dataset.recipe],direct=canDirectCraft(recipe,counts),auto=document.getElementById('craft-auto-v14')?.checked!==false;button.classList.toggle('craft-ready-v14',direct);button.classList.toggle('craft-auto-possible-v14',!direct&&auto);const status=button.querySelector('[data-craft-state]');if(status)status.textContent=direct?'Sẵn sàng':auto?'Có thể tự chuẩn bị':'Thiếu nguyên liệu'})}
function renderHUD(){
 if(!hud)return;
 ...
 hud.classList.toggle('show',active);renderCrafting();
}
...
async function reconnectServer(){const data=await loadState();if(data){if(active)generateTerrain();notify('✅ Đã kết nối lại và đồng bộ thế giới.')}return Boolean(data)}
async function enterSurvival(){
 ...
 notify(serverOnline?'🏕️ Đã vào đảo sinh tồn. Có thể đào cả móng đá đến lớp đá nền cuối cùng.':'🏕️ Đang xem thế giới ngoại tuyến. Hãy kết nối lại trước khi đào.');renderHUD();startTicks();
}
...
async function craftItem(recipeId){const button=document.querySelector(`#survival-craft-v11 [data-recipe="${CSS.escape(recipeId)}"]`);if(button){button.disabled=true;button.classList.add('crafting-v14')}try{const autoCraft=document.getElementById('craft-auto-v14')?.checked!==false;const data=await api('/api/survival/craft',{method:'POST',body:JSON.stringify({recipeId,autoCraft})});if(data.state)applyState(data.state);if(Array.isArray(data.inventoryItems))houseState.inventory=[...data.inventoryItems];if(data.output&&TOOL_INFO[data.output]&&!hotbarSlots.includes(data.output)){const empty=hotbarSlots.indexOf(null);if(empty>=0)hotbarSlots[empty]=data.output}renderHotbarUI();updateUI();renderCrafting();notify(`🛠️ ${data.message}`)}catch(error){notify(`❌ ${error.message}`,true)}finally{if(button){button.disabled=false;button.classList.remove('crafting-v14')}}}
...
async function placeBlock(target,hit){
 ...
 const placeable=['floor_grass','floor_dirt','survival_stone','survival_deepslate','survival_log','survival_plank','survival_torch'];if(!placeable.includes(itemId)){notify('🧱 Vật phẩm này không dùng để xây trong sinh tồn.',true);return true}
...
function setup(){ensureUI();setupKeys();loadState();document.addEventListener('contextmenu',event=>{if(active)event.preventDefault()});window.addEventListener('beforeunload',()=>syncVitals(true));window.SurvivalV14={handleAction,enter:enterSurvival,leave:leaveSurvival,reconnect:reconnectServer,isActive:()=>active,isFoundationHole:()=>false};window.SurvivalV13=window.SurvivalV14;window.SurvivalV11=window.SurvivalV14}
