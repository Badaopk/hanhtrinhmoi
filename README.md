...
- Sửa đúng lỗi tiến trình Node.js bị sập khi bộ quét nhiệm vụ dùng `user.history.push(...)` nhưng truy vấn không lấy trường `history`.
- Tác vụ nền được tách thành mô-đun, có khóa chống chạy chồng, `try/catch` toàn phiên và cô lập lỗi từng tài khoản.
- 2.980 bài học có lý thuyết theo đặc thù môn học, gồm khái niệm, quy trình, ví dụ, từ khóa, lỗi thường gặp và câu tự kiểm tra.
- Chế tạo sinh tồn dùng công thức kiểu Minecraft; có chế độ tự làm Ván/Gậy từ Gỗ và hiển thị số nguyên liệu trong ba lô.
- Sửa vỡ giao diện Cờ Vua, Caro, Cờ Vây và Cờ Tỷ Phú do CSS toàn cục; Cờ Vây hiện bàn ngay khi mở trang.

Xem `UPGRADE_V14.md`, `docs/ARCHITECTURE_V14.md` và `docs/TEST_REPORT_V14.md`.
